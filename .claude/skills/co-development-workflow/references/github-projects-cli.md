...
3. Set the **column field** to the workflow's status field - the custom **`Stage`**, NOT the
   ...
   columns ARE that field's options (Backlog, Ready, In progress, In review, Done). The
...
5. **Always add a one-line description to each column** so the board reads clearly (the grey
   ...
   Do this in the UI, NOT via the API: `updateProjectV2Field` can only set option
...
- Repo: `patr7257/RoboRally02162`. Integration branch: `main` (also production: PRs only, never
  push directly, never self-merge). GitHub identity: `patr7257`.
- Board: GitHub Project #9 "RoboRally dev board", owner `patr7257`, project id
  `PVT_kwHOCbc9N84BeXyr`. Column field is the built-in Status, field id
  `PVTSSF_lAHOCbc9N84BeXyrzhYyiGg`; options: Backlog `be1fbdb4` / Ready `2a994016` /
  In progress `ab3e92fa` / In review `a5eb9ded` / Done `1106ac2f`.
- Move a card:
  ```
  gh project item-edit --id <ITEM_ID> --project-id PVT_kwHOCbc9N84BeXyr \
    --field-id PVTSSF_lAHOCbc9N84BeXyrzhYyiGg --single-select-option-id <OPTION_ID>
  ```
- Find an item id:
  ```
  gh project item-list 9 --owner patr7257 --format json \
...
- **Project *views* are UI-only (see section 3b).** The CLI can create issues, add items, and move cards, but
